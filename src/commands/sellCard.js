...
const { Cards, UserContainer } = require("../db"); // Adjust path if needed
const { getRarityStars } = require("../functions"); // Adjust path if needed
const { getAscIcon } = require("../commands/inv_cards"); // Adjust path if needed
...
async function sellCard(message) {
  ...
  // 1. Fetch User (To check equipped card)
  ...
  if (args[1] && !args[1].startsWith("-") && !isNaN(parseInt(args[1]))) {
    ...
    const userCards = await Cards.find({ ownerId: userId }).sort({ _id: 1 }).populate("masterData");
    
    // Find specific card by UID (or index)
  ...
  // CASE B: Sell by Filters (e.g. !sell -r 1)
    ...
    if (!searchName && !searchType && !searchRarity && !searchFranchise && !searchAscension) {
      return message.reply("⚠️ Usage: `!sell [uid]` OR `!sell -r 1 -n [Card name]` (filters).");
    }
  ...
  // ---------------------------------------------------------
  // 3. SAFETY CHECKS (Favorites & Equipped)
  // ---------------------------------------------------------
  
  // Filter out locked/equipped cards
  const initialCount = cardsToSell.length;
  cardsToSell = cardsToSell.filter(c => {
    // Check Favorite
    if (c.fav) return false;
    // Check Equipped
    if (userProfile.selectedCard && userProfile.selectedCard.toString() === c._id.toString()) return false;
    return true;
  });

  if (cardsToSell.length === 0) {
    if (initialCount > 0) return message.reply("⚠️ All selected cards are either **Favorite** (❤️) or **Equipped** and cannot be sold.");
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(`Are you sure you want to sell **${cardsToSell.length}** cards?\n\n${descriptionStr}`)
    .addFields({ 
      name: "Total Rewards", 
  ...
  collector.on("collect", async (interaction) => {
    ...
    if (interaction.customId === "confirm_sell") {
      try {
        // Double check inventory hasn't changed (optional but safe)
        // Perform Deletion
        ...
        const successEmbed = new EmbedBuilder()
          ...
          .setDescription(`Sold **${cardsToSell.length}** cards.\n\n${descriptionStr}`)
          .addFields({ 
        ...
        await interaction.followUp({ content: "❌ An error occurred while selling.", ephemeral: true });
      }
    }
  });
  ...
  collector.on("end", (collected, reason) => {
    if (reason === "time") {
      replyMsg.edit({ content: "⚠️ Sell confirmation timed out.", components: [] }).catch(() => {});
    }
  });
}
