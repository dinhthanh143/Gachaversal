...
async function starterChoice(message) {
  try {
    ...
    starters.sort((a, b) => STARTER_IDS.indexOf(a.pokeId) - STARTER_IDS.indexOf(b.pokeId));
    ...
    const generateEmbed = (index) => {
      ...
      return new EmbedBuilder()
        ...
        .setDescription(`Select a unit to begin your journey. They will join you as a **4-Star** companion.\n\n⚠️ **You have 5 minutes to decide.**`)
        .setAuthor({ 
    ...
    const msg = await message.reply({ embeds: [generateEmbed(currentIndex)], components: [row] });

    // 4. Collector (5 Minutes)
    const filter = (i) => i.user.id === userId;
    const collector = msg.createMessageComponentCollector({ filter, time: TIMEOUT_MS, componentType: ComponentType.Button });
    ...
    collector.on("end", async (collected, reason) => {
      if (reason !== "confirmed") {
        try {
            ...
            await msg.edit({ 
                content: "⏳ **Time ran out.** Guess you're not interested in this bot.\n*Account data has been deleted.*", 
                embeds: [], 
                components: [] 
            });
        } catch (delErr) {
