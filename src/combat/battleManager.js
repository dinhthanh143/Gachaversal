...
function getSkillValues(masterData, rarity) {
  if (!masterData.skill || !masterData.skill.values || masterData.skill.values.length === 0) {
    return [1.0];
  }
  const rarityIndex = Math.max(0, rarity - 1);
  return masterData.skill.values.map((valArray) => {
    if (Array.isArray(valArray)) {
      return valArray[rarityIndex] !== undefined ? valArray[rarityIndex] : valArray[valArray.length - 1];
    }
    return valArray;
  });
}
...
function getSkillSafe(skillName) {
  ...
  return {
    name: "Unknown",
    execute: (att, def) => ({ damage: 0, log: `${att.name} tried to attack but tripped!` }),
  };
}

// ----------------------------------------------------
// ✅ HELPER: Format Drop Embed Field
// ----------------------------------------------------
...
async function skipBattle(message, inputTimes = 1) {
  ...
  try {
    ...
    // ✅ Call shared reward logic
    const report = await processBattleRewards(userId, user, user.selectedCard, areaData.stages[stageId], requestedRuns);
...
async function startBattle(message) {
  ...
  try {
    ...
    if (!user || !user.selectedCard) {
      return battleMsg.edit({ content: "❌ You need to `!select` a card first!", embeds: [] });
    }
    if (user.stam < STAMINA_COST) {
      return battleMsg.edit({ content: `⚠️ Not enough Stamina! (Need ${STAMINA_COST} ⚡)`, embeds: [] });
    }

    const areaId = user.dungeon.currentArea;
    const stageId = user.dungeon.currentStage;

    if (areaId === 0) return battleMsg.edit({ content: "⚠️ Use `!area 1` first!", embeds: [] });
    const areaData = DUNGEON_AREAS[areaId];
    if (!areaData || !areaData.stages[stageId]) return battleMsg.edit({ content: "❌ Invalid Stage data.", embeds: [] });
    ...
    if (!masterData) masterData = await Index.findOne({ pokeId: playerCard.cardId });
      ...
      if (unit.skill && unit.skill.name.includes("[PASSIVE]")) {
        ...
        if (unit.effects.length > 0) {
          unit.effects.forEach((e) => {
            if (e.name === unit.skill.name || e.name === "Wind's Edge") e.turns = 999;
          });
        }
      }
    };
    ...
    await wait(2000);
    updateBars();
    let embedData = await createBattleEmbed(player, player.type, enemy.type, playerCard.rarity, mobTemplate.rarity, enemy, logs, turn, null, battleTitle);
    ...
    while (!battleOver && turn <= MAX_TURNS) {
      ...
      embedData = await createBattleEmbed(player, player.type, enemy.type, playerCard.rarity, mobTemplate.rarity, enemy, logs, turn, battleBuffer, battleTitle);
      await battleMsg.edit({ embeds: [embedData.embed], files: embedData.files });
      await wait(1500);
      ...
      const runTurnLifecycle = async (actor, target) => {
        ...
        if (isStunned) {
            ...
            if (actor.energy >= skillCost && canUseSkill) {
                ...
                const skillResult = skillLogic.execute(actor, target, actor.skill.values);
            ...
            if (target.stats.hp > 0) {
                ...
                if (!isSilenced && !isPassiveSkill) {
                    actor.energy = Math.min(skillCost, actor.energy + 25);
                }
      ...
      if (gameOver) { battleOver = true; playerWon = first === player; break; }
      gameOver = await runTurnLifecycle(second, first);
      if (gameOver) { battleOver = true; playerWon = second === player; break; }
      turn++;
    }
    ...
    if (playerWon) {
      ...
      // ✅ Call shared reward logic
      const report = await processBattleRewards(userId, user, playerCard, areaData.stages[stageId], 1);

      // Handle Progression Unlocks
      let unlockMsg = "";
      let bonusGold = 0;
      let bonusTickets = 0;
      const isProgression = areaId === user.dungeon.maxArea && stageId === user.dungeon.maxStage;
      ...
      if (isProgression) {
        ...
        if (stageId === totalStagesInArea) {
          ...
          if (!unlockMsg.includes("Milestone")) unlockMsg = "🔓 **Stage Cleared!** Next stage unlocked.";
        }
      }
      user.gold += bonusGold;
      if (bonusTickets > 0) {
      ...
      // Save again for bonus gold/progression
      await user.save();
      ...
      const winEmbed = new EmbedBuilder()
        ...
        .setTitle(`🏆 Victory!`)
        .setDescription(`**Congrats ${message.author.username}!**, Your **${player.name}** Cleared stage **${areaId}-${stageId}**!`)
        .addFields({
          name: "Rewards",
          value: `🪙 **+${report.gold + bonusGold}** \n ${xpIcon} **+${report.cardXp}** (Card)\n 🆙 **+${report.accountXp}** (Account)`,
          inline: true,
        })
        .setFooter({ text: `Type !next stage or move to the next Area to dive deeper into the Dungeon.` });

      if (report.levelsGained > 0)
        winEmbed.addFields({ name: "Card Level Up!", value: `🆙 ${player.name} is now **Lv.${playerCard.level}**!`, inline: true });
      
      // ✅ Add Drops Fields (Cards/Items)
      const dropFields = formatDrops(report);
      winEmbed.addFields(dropFields);

      if (isProgression && unlockMsg)
        winEmbed.addFields({ name: "Progression", value: unlockMsg, inline: false });
      ...
      if (report.accountLevelUp) {
        const lvlEmbed = new EmbedBuilder()
          ...
          .addFields(
            { name: "⚡ Stamina Refilled", value: `Current: **${user.stam}/${user.stamCap}** (+2 Cap)`, inline: true },
            { name: "💰 Bonus Gold", value: `+${report.lvlUpGold} 🪙`, inline: true },
            { name: "🎫 Bonus Tickets", value: `+${report.lvlUpTickets} Tickets`, inline: true }
          );
